...
(define (reselect-spinner id items layout listener) (list "reselect-spinner" id items layout listener))
...
(define (widget-get-callback w)
  (cond
   ...
   ((equal? (widget-type w) "reselect-spinner") (spinner-listener w))
        ...
        (for-each
         (lambda (event)
           (when (or
                  ...
                  (equal? (list-ref event 0) "encrypt")
                  (equal? (list-ref event 0) "decrypt")
        ...
        (let ((events
               (cond
                ...
                ((equal? (callback-type cb) "spinner")
                 ((callback-fn cb) (car args)))
                ((equal? (callback-type cb) "reselect-spinner")
                 ((callback-fn cb) (car args)))
